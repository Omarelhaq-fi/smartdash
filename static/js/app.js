...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function apiRequest(url, method = 'GET', body = null) {
        ...
        try {
            ...
            // Handle cases where response might be empty (e.g., on a successful DELETE)
    ...
    function loadPageData(pageId) {
        switch(pageId) {
            ...
            case 'gym': loadTabData('gym', 'gym-today'); break; // Load default tab
            case 'basketball': loadTabData('basketball', 'bball-stats'); break; // Load default tab
        }
    }
    ...
    function handleGlobalClick(e) {
        // ** MODAL FIX: Only close if the click is on the modal background itself **
    ...
    // They are included here for completeness but are the same as before.
    ...
    function openExerciseModal(exerciseId = null) {
        // This function would fetch the exercise if ID is provided for editing
    ...
    async function saveExercise() {
        ...
        // Add logic for PUT request if editing
    ...
    async function logShot(made) {
        await apiRequest('/api/basketball/shots', 'POST', { ...tempShotData, made });
    ...
    async function renderPlayerStats() {
        ...
        let tableHTML = `<div class="bg-gray-800 p-4 rounded-lg"><h3 class="text-xl font-semibold mb-4">Player Stats</h3><div class="overflow-x-auto"><table class="w-full text-left"><thead><tr><th>Player</th><th>PTS</th><th>FGM/A</th><th>AST</th></tr></thead><tbody>`;
